# coding=utf-8
# IWSLT 2017 dataset, with updated links that correctly download from google drive.
# Using only the 2010 and 2015 test sets for validation and testing, following Libovicky et al. 2021
...
MULTI_URL = "https://drive.google.com/uc?id=12ycYSzLIG253AFN35Y6qoyf9wtkOjakp"
...
class IWSLT217(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        ...
        if self.config.is_multilingual:
            ...
            # data_dir = os.path.join(dl_dir, "2017-01-trnmted/texts/DeEnItNlRo/DeEnItNlRo/DeEnItNlRo-DeEnItNlRo")
            years = [2010]
        else:
            # bi_url = f"https://wit3.fbk.eu/archive/2017-01-trnted/texts/{source}/{target}/{source}-{target}.tgz"
            bi_url = f"https://drive.google.com/uc?id=1gFeuPTRc3RB4DhJEkhr8O-a8PObM7Ix2"
            dl_dir = dl_manager.download_and_extract(bi_url)
            data_dir1 = os.path.join(dl_dir, "2017-01-trnted/texts", source, target, f"{source}-{target}.tgz")
            data_dir = dl_manager.extract(data_dir1)
            data_dir = os.path.join(data_dir, f"{source}-{target}")
            ...
            datasets.SplitGenerator(
                ...
                gen_kwargs={
                    "source_files": [
                        os.path.join(
                            data_dir,
                            "IWSLT17.TED.tst2010.{}.{}.xml".format(self.config.pair, source),
                        )
                    ],
                    "target_files": [
                        os.path.join(
                            data_dir,
                            "IWSLT17.TED.tst2010.{}.{}.xml".format(self.config.pair, target),
                        )
                    ],
                    "split": "dev",
                },
