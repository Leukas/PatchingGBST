    ...
    def __init__(self, pretrained_model, freeze, bert_config=None):
        ...
        if pretrained_model == 't5-small':
            self.gbst = GBST(
                ...
                no_conv=True,
                max_block_size=4)
            self.bert.lm_head = nn.Linear(self.t5.model_dim, 384)
        else:
            self.gbst = GBST(
                num_tokens=self.vocab_size, # 384
                dim=self.t5.model_dim,
                no_conv=True,
                max_block_size=4)
...
class BertWithGBSTFull(nn.Module):
    def __init__(self, pretrained_model, freeze, bert_config=None, causal=False):
        super().__init__()
        if bert_config is not None:
            enc = BertModel(bert_config)
            bert_config.is_decoder = True
    ...
    def decode(self, decoder_input_ids, decoder_attention_mask, encoder_hidden_states, encoder_attention_mask):
        ...
        dec_char_embs = self.decoding_char_embs(decoder_input_ids)
        hidden_proj = self.decoding_proj_layer(out.hidden_states[-1])
        repeated_out = rearrange(hidden_proj, 'b n (m d) -> b (n m) d', m=self.ds_factor)
