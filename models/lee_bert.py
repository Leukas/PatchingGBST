...
class BertWithLeeFull2(nn.Module):
    def __init__(self, pretrained_model, freeze, bert_config=None, causal=True, ds_factor=4):
        super().__init__()
    ...
    @classmethod
    def from_pretrained(cls, reload_folder, freeze=False, config=None, causal=True, ds_factor=4):
        model = cls(None, freeze, config, causal, ds_factor)
        state_dict_path = reload_folder + "/pytorch_model.bin"
        model.load_state_dict(torch.load(state_dict_path))
        return model

    def forward(self, input_ids, attention_mask=None, decoder_input_ids=None, decoder_attention_mask=None, labels=None):
        embed, mask = self.enc_lee(input_ids, mask=attention_mask.bool() if attention_mask is not None else None)
        ...
        dec_embed, dec_mask = self.dec_lee(buffered_dec_ids, mask=buffered_dec_mask.bool() if decoder_attention_mask is not None else None)
        out = self.bert(
            inputs_embeds=embed, 
            attention_mask=mask,
    ...
    def decode_lstm_generate(self, decoder_input_ids, decoder_attention_mask, encoder_hidden_states, encoder_attention_mask, lstm_hidden_state, past_key_values):
        dec_embed, dec_mask = self.dec_lee(decoder_input_ids, mask=decoder_attention_mask.bool() if decoder_attention_mask is not None else None)
        
        dec_embed = dec_embed[:, -1:]
